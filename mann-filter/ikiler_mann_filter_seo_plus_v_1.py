...
def load_all_collections() -> Dict[str, int]:
    """
    Shopify'daki TÜM custom collection'ları yükle
    ...
    try:
        ...
        data = shopify_get(f"{BASE}/custom_collections.json?limit=250", timeout=30)

        if not data or not data.get("custom_collections"):
            log("⚠ Shopify'da koleksiyon bulunamadı", "WARN")
            return {}

        for coll in data["custom_collections"]:
...
def add_product_to_collections(
    ...
    if vehicles:
        for brand_raw in vehicles.keys():
            ...
            # VW özel durumu
            if brand_raw.upper() in ["VW", "VOLKSWAGEN"]:
                possible_names.extend(["VW", "Volkswagen"])
    ...
    if added_count > 0:
        ...
        log(f"  ⚠ Hiçbir koleksiyona eklenemedi", "WARN")
...
def main():
    ...
    products_to_process = mann_products[:MAX_PRODUCTS]

    success = 0
    failed = []

    log(f"\n📦 {len(products_to_process)} ürün işlenecek...\n")
